...
def toeplitz_conv(tensor, input, order):
   
    assert tensor.name == 'BlockTT'
    ...
    #reshape input
    inp_unf = torch.nn.functional.unfold(inp, (tensor.kernel_size[0], tensor.kernel_size[1]))

    #reshape output and multiply
    out_unf = inp_unf.transpose(1, 2).matmul(w.t()).transpose(1, 2)

    #reshape output
    out = out_unf.view(inp.shape[0], w.shape[0], H-tensor.kernel_size[0]+1, W-tensor.kernel_size[1]+1)

    return out

def my_tensor_conv(tensor, input, order):
    assert tensor.name == 'BlockTT'
    output = toeplitz_conv(tensor,input, order)
    saved_tensors = []
